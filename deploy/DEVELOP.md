...
仅在新的服务器部署，不修改生产服务器，不合并 main。支持 Apple Web Push 与普通网页系统通知；Windows 可使用网页或安装 PWA，桌面托盘壳与国内 Android 厂商推送不在本次范围内。
...
3. 在邮件模板中选中一条“设为主推送”。每个文档最多一个，可取消；不同文档互不影响。升级自动保留旧选择。邮件仍按原配置发送，推送给模板所属帐号开启通知的所有设备，不按邮件接收人推断用户。
4. iPhone（iOS 16.4+）用 Safari 打开新的 HTTPS 网站，分享 → 添加到主屏幕。
5. 从桌面图标打开，重新登录。在首页打开“允许通知”开关，允许系统权限。
6. 开启成功后自动向当前设备发送“消息推送启动成功！”，不发邮件，不执行文档规则。重复刷新或同步订阅不重复发送；关闭再开启会重新发送。
...
普通网页的“允许通知”在用户授权后通过系统通知显示消息，需要保持至少一个页面打开；浏览器可能节流后台轮询，不保证即时送达。iPhone 普通标签页或不暴露通知 API 的应用壳使用页内提醒，界面会明确显示此模式。没有新增 Windows/Android 原生壳或厂商 SDK；壳关闭后的后台推送仍需原生适配。iPhone 主屏幕应用继续走 Apple 后台推送，不依赖页面轮询。关闭本设备开关不影响邮件或其他设备。

## 4. 真机验收清单

- 开启通知后确认自动成功消息显示；接口中的 accepted 只表示 Apple 受理，不表示用户已看见。
...
- 在另一台 iPhone 登录同一帐号并开启通知，验证多设备；关闭“允许通知”开关不影响另一设备。
...
- 通知只在触发时生成。切换主推送、首次开启不补推历史记录。普通网页恢复连接时继续已保存的游标，只显示仍在一小时有效期内的未读到的通知。
- Web Push 使用独立持久化队列，每次最多处理 20 项；临时连接错误、429、5xx 最多尝试 5 次，重试间隔 30/60/120/240 秒。每条通知有效期 1 小时，发给 Apple 的 TTL 为剩余有效期。
- 404/410 清理失效订阅；其他永久性错误停止重试，可通过 config 接口查看最近投递状态。处理配置后可关闭再开启通知。
...
均须登录；写入携带 `Content-Type: application/json` 与 `X-Requested-With: WeeklyReport`。以下接口按帐号隔离；读取主推送须用 `X-Document-ID` 或 `document_id` 查询参数指定文档。

| 接口 | 内容 |
| --- | --- |
| `GET /api/me/primary-trigger` | 指定文档的选择与模板标题 |
| `PUT /api/me/primary-trigger` | `{"document_id":"…","trigger_id":"…"}`；保留 document_id，trigger_id 为 null 取消该文档选择 |
| `GET /api/push/config` | 是否启用、公钥、当前设备登记及最近投递状态 |
| `POST /api/push/subscriptions` | 浏览器 `PushSubscription.toJSON()`；先读取 config 建立 HttpOnly 设备 Cookie；新订阅自动排队当前设备的成功通知 |
| `DELETE /api/push/subscriptions/{id}` | 仅删除本帐号的指定订阅 |
| `GET /api/notifications?after=0` | 最多 100 条本人业务通知，返回 `next_cursor`；不包含测试消息 |

普通网页每 15 秒增量读取通知列表，使用系统通知 API 展示邮件标题，不建立 Google 推送订阅。开启时用 `after=latest` 获取初始游标，避免补发旧消息；过期通知跳过。多个标签页通过 Web Locks 协调，关闭开关同步至同一浏览器的标签页。没有 Web Locks 的旧环境仅在可见页面读取，不保证多窗口去重。通知状态 `accepted` 指 Apple 受理，没有实现已读回执。
