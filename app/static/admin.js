...
(async()=>{
  ...
  form.onsubmit=async event=>{event.preventDefault();const button=form.querySelector('[type=submit]');button.disabled=true;try{await api('admin/users'+(editing?'/'+editing.id:''),editing?'PUT':'POST',Object.fromEntries(new FormData(form)));if(editing?.id===account.id){location.reload();return;}form.hidden=true;await load();toast('用户已保存');}catch(error){toast(error.message);}finally{button.disabled=false;}};
